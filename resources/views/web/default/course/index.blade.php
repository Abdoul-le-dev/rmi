...
@section('content')
    <section class="course-cover-container {{ empty($activeSpecialOffer) ? 'not-active-special-offer' : '' }}">
        ...
        <div class="cover-content pt-40">
            @if(!empty($promo) && !$hasBought && $course->slug == 'devenez-trader-pro' && $promo->type == 'Purchase Module')
                <div class="promo_div">
            ...
            <div id="subscriptionCard" class="card w-50" style="position: absolute; top: 18rem; height: auto; max-width: 90%; z-index:5;">
                <div class="card-body p-0">
                    <div class="row no-gutters">
                        <div class="col-md-5">
                            <img src="{{ $course->getImage() }}" class="img-fluid img-cover-2" alt="" style="width: 100%; height: auto;">
                            <div class="mt-3 px-3 py-2">
                                <h3 class="h5">{{ $course->title }}</h3>
                            </div>
                        </div>
                        <div class="col-md-7 d-flex flex-column justify-content-center">
                            <h2 class="subscribe_title text-danger text-center">{{trans('new_translations.subscribe_first')}}</h2>
                            <div class="d-flex justify-content-center mt-4">
                                <button id="closeCardBtn" class="btn btn-outline-danger">{{trans('new_translations.close')}}</button>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="mt-35">
                        ...
                        <div class="tab-content" id="nav-tabContent">
                            <div class="tab-pane fade {{ (empty(request()->get('tab','')) or request()->get('tab','') == 'information') ? 'show active' : '' }} " id="information" role="tabpanel"
                                 ...
                                 @include(getTemplate() . '.course.tabs.information', ['purchaseModels' => $purchaseModels, 'hasBought' => $hasBought])
                            </div>
                ...
                <div class="rounded-lg shadow-sm">
                    ...
                    <div class="px-20 pb-30">
                        <form action="/cart/store" method="post">
                            ...
                            @if($course->price > 0 && $course->type == 'webinar')
                            ...
                            <div class="mt-20 d-flex flex-column">
                                ...
                                @elseif(!empty($course->price) and $course->price > 0 and $course->type == 'webinar')
...
@push('scripts_bottom')
    ...
    {{-- <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (distance > 0) {
                ...
                clearInterval(timer);
                document.querySelector('.promo_div').style.display = 'none';
            }
            }
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const promoEndDate = "{{ $promo ? \Carbon\Carbon::parse($promo->end_date)->toIso8601String() : '' }}";
