    ...
    <section class="mt-25">
        ...
        <form method="GET" action="/panel/marketing/affiliates" class="panel-section-card py-20 px-25 mt-20 mb-2">
            <div class="search-inputs mb-3 d-flex">
                <input type="text" id="searchFullName" name="searchFullName" class="form-control mb-2 mr-2 w-25" placeholder="Search by Full Name" value="{{ request('searchFullName') }}">
                <input type="text" id="searchEmail" name="searchEmail" class="form-control mb-2 w-25 mr-2" placeholder="Search by Email" value="{{ request('searchEmail') }}">
                <select id="searchPurchaseStatus" name="searchPurchaseStatus" class="form-control mb-2 w-25">
                    <option value="both" {{ request('searchPurchaseStatus') === 'both' ? 'selected' : '' }}>All</option>
                    <option value="made" {{ request('searchPurchaseStatus') === 'made' ? 'selected' : '' }}>Made Purchase</option>
                    <option value="not_made" {{ request('searchPurchaseStatus') === 'not_made' ? 'selected' : '' }}>Did not purchase</option>
                </select>
            </div>

            <div class="d-flex">
                <input type="date" id="searchStartDate" name="searchStartDate" class="form-control mb-2 mr-2 w-25" placeholder="Start Date" value="{{ request('searchStartDate') }}">
                <input type="date" id="searchEndDate" name="searchEndDate" class="form-control mb-2 mr-2 w-25" placeholder="End Date" value="{{ request('searchEndDate') }}">
                        ...
                        <table class="table text-center custom-table">
                            ...
                            @foreach($affiliates as $affiliate)
                                <tr>
                                    ...
                                    <td class="text-left">
                                            <div class=" ml-5">
                                                <span class="d-block font-weight-500">{{ $affiliate->referredUser->email }}</span>
                                            </div>
                                    </td>
                                    ...
                                    <!-- <td data-date="{{ \Carbon\Carbon::parse($affiliate->created_at)->format('Y-m-d') }}">
                                        {{ \Carbon\Carbon::parse($affiliate->created_at)->format('m/d/Y') }}
                                    </td> -->
                                </tr>
                            @endforeach
                            </tbody>
