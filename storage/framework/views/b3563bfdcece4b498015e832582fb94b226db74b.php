...
<?php $__env->startSection('content'); ?>
    <section class="course-cover-container <?php echo e(empty($activeSpecialOffer) ? 'not-active-special-offer' : ''); ?>">
        ...
        <div class="cover-content pt-40">
            <?php if(!empty($promo) && !$hasBought && $course->slug == 'devenez-trader-pro' && $promo->type == 'Purchase Module'): ?>
                <div class="promo_div">
                    <div class="row promo_row">
                        <div class="col-8 offer_text">
                            <h3 class="promo_h3"><?php echo e($promo->title ?? 'No Active Promo'); ?></h3>
                            <span class="promo_offer_text_span"><?php echo e($promo->description ?? ''); ?></span>
                        </div>
                    ...
                    <div class="percentage_div">
                        <span><?php echo e($promo->percentage ?? 0); ?>%</span>
    ...
    <?php if(session('showCard')): ?>
        <div class="d-flex justify-content-center">
            <div id="subscriptionCard" class="card w-50" style="position: absolute; top: 18rem; height: auto; max-width: 90%; z-index:5;">
                <div class="card-body p-0">
                    <div class="row no-gutters">
                        <div class="col-md-5">
                            <img src="<?php echo e($course->getImage()); ?>" class="img-fluid img-cover-2" alt="" style="width: 100%; height: auto;">
                            <div class="mt-3 px-3 py-2">
                                <h3 class="h5"><?php echo e($course->title); ?></h3>
                            </div>
                        </div>
                        <div class="col-md-7 d-flex flex-column justify-content-center">
                            <h2 class="subscribe_title text-danger text-center"><?php echo e(trans('new_translations.subscribe_first')); ?></h2>
                            <div class="d-flex justify-content-center mt-4">
                                <button id="closeCardBtn" class="btn btn-outline-danger"><?php echo e(trans('new_translations.close')); ?></button>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="mt-35">
                        ...
                        <div class="tab-content" id="nav-tabContent">
                            <div class="tab-pane fade <?php echo e((empty(request()->get('tab','')) or request()->get('tab','') == 'information') ? 'show active' : ''); ?> " id="information" role="tabpanel"
                                 ...
                                 <?php echo $__env->make(getTemplate() . '.course.tabs.information', ['purchaseModels' => $purchaseModels, 'hasBought' => $hasBought], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
                            </div>
                ...
                <div class="rounded-lg shadow-sm">
                    ...
                    <div class="px-20 pb-30">
                        <form action="/cart/store" method="post">
                            ...
                            <?php if($course->price > 0 && $course->type == 'webinar'): ?>
                            ...
                            <div class="mt-20 d-flex flex-column">
                                ...
                                <?php elseif(!empty($course->price) and $course->price > 0 and $course->type == 'webinar'): ?>
...
<?php $__env->startPush('scripts_bottom'); ?>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            // Check if promo exists and get the promo end date
            const promoEndDate = "<?php echo e($promo ? \Carbon\Carbon::parse($promo->end_date)->toIso8601String() : ''); ?>";

            // If promo exists, proceed with countdown logic
            if (promoEndDate) {
                // Convert promo end_date from UTC to JavaScript Date object (in UTC)
                const endDateUTC = new Date(promoEndDate); // End date in UTC

                // Benin is UTC +1, so we need to adjust the end time to Benin time
                const endDateBenin = new Date(endDateUTC.getTime()); // Add 1 hour to match Benin time

                function updateCountdown() {
                    // Get current UTC time
                    const nowUTC = new Date().getTime(); // This is in UTC time
                    const distance = endDateBenin.getTime() - nowUTC; // Time left in milliseconds (using UTC)
                    ...
                    if (distance > 0) {
                        ...
                        // Reload only if not reloaded already
                        if (!localStorage.getItem('promoExpiredReloaded')) {
                            localStorage.setItem('promoExpiredReloaded', 'true');
                            location.reload();
                        }
                    }
                }

                localStorage.removeItem("promoExpiredReloaded");
                const timer = setInterval(updateCountdown, 1000);
                updateCountdown();
            }
            else {
                // Remove flag if promo is completely removed
                localStorage.removeItem("promoExpiredReloaded");
            }
        });
    </script>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            var closeButton = document.getElementById('closeCardBtn');
            var subscriptionCard = document.getElementById('subscriptionCard');

            if (closeButton && subscriptionCard) {
...
<?php echo $__env->make(getTemplate().'.layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/rmi/resources/views/web/default/course/index.blade.php ENDPATH**/ ?>