...
(function ($) {
    ...
    $("body").on("change", ".js-switch", function () {
        ...
        if (this.checked) {
            $(".js-switch").each(function () {
                ...
                var switcher_input = switcher_parent.find(
                    'input[type="checkbox"]'
                );
    ...
    $("body").on("click", ".edit_question", function (e) {
        ...
        loadingSwl();
        $.get(
            "/panel/quizzes-questions/" + question_id + "/edit",
            function (result) {
                if (result && result.html) {
                    var $html =
                        '<div id="editQuestion">' + result.html + "</div>";
                    Swal.fire({
                        html: $html,
                        showCancelButton: false,
                        showConfirmButton: false,
    ...
    $("body").on("click", ".js-submit-quiz-form", function (e) {
        ...
        $.post(action, data, function (result) {
            if (result && result.code === 200) {
                console.log("Success! Showing Swal...");
                Swal.fire({
                    icon: "success",
                    html:
                    ...
                    console.log("Swal closed, redirecting...");
                    console.log("le lien : ", result.redirect_url);

                    if (
                        result.redirect_url &&
                        result.redirect_url.trim() !== ""
                    ) {
    ...
    $("body").on("change", ".js-ajax-webinar_id", function (e) {
        ...
        $.get(path, function (result) {
            if (result && result.chapters) {
                var html =
                    '<option value="">' + quizzesSectionLang + "</option>";
    ...
    $(document).ready(function () {
        ...
        function updateToDatabase(table, quizId, idString) {
            $.post(
                "/panel/quizzes/" + quizId + "/order-items",
                {
                    table: table,
                    items: idString,
                },
