...
class Setting extends Model implements TranslatableContract
{
    ...
    {
        // Check if the value is already cached or set in the static variable.
        if (!isset($static)) {
            // Use a variable to cache the result and prevent multiple queries.
            $static = cache()->remember('settings.' . $name, 24 * 60 * 60, function () use ($name) {
                // Retrieve the setting from the database only once.
                $setting = self::where('name', $name)->first();

                // Return null if no setting is found.
    ...
    // static function getGeneralSecuritySettings($key = null)
    // {
    //     Log::debug('Entering getGeneralSecuritySettings', ['key' => $key, 'trace' => debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS)]);

    //     $result = self::getSetting(self::$generalSecuritySettings, self::$generalSecuritySettingsName, $key);

    //     Log::debug('Exiting getGeneralSecuritySettings', ['key' => $key, 'result' => $result]);
