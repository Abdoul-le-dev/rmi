...
class Subscribe extends Model implements TranslatableContract
{
    ...
    public static function getActiveSubscribe($userId)
    {
        ...
        // Log::info('activePlan',['data'=>$activePlan]);
    ...
    public static function getDayOfUse($userId)
    {
        $lastSubscribeSale = Sale::where('buyer_id', $userId)
            ...
            if ($lastSubscribeSale) {
                ...
                $subscriptionTimestamp = $lastSubscribeSale->created_at; // Unix timestamp

                $diffInSeconds = $currentTimestamp - $subscriptionTimestamp;
                $days = floor($diffInSeconds / (24 * 60 * 60)); // Calculate full days
                $hours = floor(($diffInSeconds % (24 * 60 * 60)) / (60 * 60)); // Remaining hours
