...
class SaleController extends Controller
{
    public function index(Request $request)
    {
        ...
        $sales = $salesQuery->orderBy('created_at', 'desc')
            ->with([
                'buyer:id,full_name,email,mobile',
                'webinar',
                'meeting',
                'product',
                'subscribe',
    ...
    private function getSalesFilters($query, $request)
    {
        ...
        if (!empty($teacher_name)) {
            $query->whereHas('seller', function ($q) use ($teacher_name) {
                $q->where('full_name', 'like', "%{$teacher_name}%");
            });
        }

        // Student name filtering
        if (!empty($student_name)) {
            $query->whereHas('buyer', function ($q) use ($student_name) {
                $q->where('full_name', 'like', "%{$student_name}%");
            });
        }

        // Type filtering
        if (!empty($type)) {
            $query->where('type', $type); // Assuming 'type' is the column for filtering
        }

        // After applying all filters, execute the query to get the results
        $results = $query->get(); // This will execute the query and return the filtered data
    
        // Log the actual results
        Log::info('query results', ['data' => $results->toArray()]);
            return $query;
    }
