...
class ForumController extends Controller
{
    ...
    public function storeTopic(Request $request)
    {
        try {
            ...
            if (!empty($forum) && $forum->checkUserCanCreateTopic($user)) {
                // Set an increased script execution time for this operation
                set_time_limit(600);
            ...
            if ($e->getCode() == 504) {
                Log::error('Timeout error in storeTopic', ['error' => $e->getMessage()]);
                return redirect()->back()->withErrors(['timeout' => 'The request timed out.']);
            }

            // General error logging
            Log::error('Error in storeTopic', ['error' => $e->getMessage()]);
            return redirect()->back()->withErrors(['error' => 'An unexpected error occurred. Please try again later.']);
        } catch (\Exception $e) {
            Log::error('Unexpected error in storeTopic', ['error' => $e->getMessage()]);
            return redirect()->back()->withErrors(['error' => 'An unexpected error occurred. Please try again later.']);
        }
    }
