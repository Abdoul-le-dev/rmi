...
use App\Models\PurchaseModelSale;
...
class WebinarController extends Controller
{
    ...
    public function course($slug, $justReturnData = false)
    {
        ...
        $promo = Promo::where('status', 'active')
            ...
            ->where('end_date', '>=', now())   // Compare full timestamp
            ->latest()
            ->first();

        // $promo = Promo::where('status', 'active')
        // ->where('start_date', '<=', now()->timezone('Africa/Porto-Novo'))  // Compare start_date with Benin adjusted time
        // ->where('end_date', '>=', now()->timezone('Africa/Porto-Novo'))   // Compare end_date with Benin adjusted time
        // ->latest()
        // ->first();

        // Log::info('promo', ['data'=>$promo]);
        // Log::info('Current time', ['now' => now()]);
        // Log::info('Promo end time', ['end_date' => $promo->end_date]);
    ...
    public function purchaseModelPay(Request $request)
    {
        try{
            ...
            $course = Webinar::where('id', $request->input('webinar_id'))->first();

            $purchase_id = $request->input('purchase_model_id');
            ...
            //         $commissionPrice += $price * $commission / 100;
            //     }
            // }
            ...
            // Create OrderItem for subscription if it exists
            if ($subscribe) {
                OrderItem::create([
                    'user_id' => $user->id,
                    'order_id' => $order->id,
            ...
            // Create OrderItem for course if it exists
            if ($course) {
                OrderItem::create([
                    'user_id' => $user->id,
                    'order_id' => $order->id,
            ...
            // Handle Free
            // Sale::createSales($orderItem1, Sale::$credit);
            // Sale::createSales($orderItem2, Sale::$credit);
